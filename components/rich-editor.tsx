...
export function RichEditor({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const cleanHtml = initialHtml.replace(
          /style="[^"]*color:[^";]*[;"]?/gi,
          ""
        );
  ...
  useEffect(() => {
    ...
    const cleanSnippet = requestInsert.replace(
      /style="[^"]*color:[^";]*[;"]?/gi,
      ""
    );
  ...
  useEffect(() => {
    const handleInsertText = (event: any) => {
      ...
      if (selection) {
        // Insert di posisi kursor saat ini
        q.insertText(selection.index, text + " ");
        q.setSelection(selection.index + text.length + 1);
      } else {
        // Insert di akhir dokumen
        const length = q.getLength();
        q.insertText(length, "\n" + text);
        q.setSelection(length + text.length + 2);
      }
    };

    window.addEventListener("insertTextToEditor", handleInsertText);

    return () => {
      window.removeEventListener("insertTextToEditor", handleInsertText);
    };
  }, []);
  ...
  return (
    <div ref={wrapperRef} className="grid gap-3">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <Input
          id="url"
          placeholder="Tempel URL sumber materi…"
          className="max-w-xl"
        />
        <Button
          variant="secondary"
